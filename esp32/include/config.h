...
// Fixed SoftAP channel — do not hop while clients are connected.
static const uint8_t AP_CHANNEL = 6;
static const uint8_t AP_MAX_CLIENTS = 4;

/**
 * SoftAP-first policy (ESP32 radio cannot reliably do SoftAP + STA scan):
 *  - Direct / hotspot drive: WIFI_AP only while SoftAP has clients.
 *  - Home join only after SoftAP has been idle (0 clients) this long.
 */
static const uint32_t SOFTAP_IDLE_BEFORE_STA_MS = 5000;
static const uint32_t STA_ATTEMPT_TIMEOUT_MS = 15000;
static const uint8_t STA_CONNECT_MAX_ATTEMPTS = 3;
/** How often to retry home join when SoftAP stays empty. */
static const uint32_t STA_IDLE_RETRY_MS = 30000;
...
static const uint32_t CAM_INTERVAL_LOW_MS = 320;
